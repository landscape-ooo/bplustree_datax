...
#ifndef SRC_SRC_JOBSCHEDULE_BinlogTrackerCli_H_
#define SRC_SRC_JOBSCHEDULE_BinlogTrackerCli_H_
#include <sys/types.h>
...
//	static bpt::bplus_tree* GetBptByMsginfo(const struct msgInfo& msg);
	...
	static string GetIpInfo(const client_t* client){
		struct sockaddr ip_adr_get;
		socklen_t ip_adr_len;
		struct sockaddr_in* addressInternet;
		ip_adr_len = sizeof(ip_adr_get);
		::getpeername(client->fd, &ip_adr_get, &ip_adr_len);
		addressInternet = (struct sockaddr_in*) &ip_adr_get;
		const string client_id = ::inet_ntoa(addressInternet->sin_addr);
		return fdfs2qq::to_string(::inet_addr(client_id.c_str()));
	}
	/****
	 * <-------event  start
	 */
	static bool _BinlogHandle_synchronous(const map<string,string>& msgstr);
...
#endif /* SRC_SRC_JOBSCHEDULE_BinlogTrackerCli_H_ */
