...
fdfs2qq_BIN=$prefix/bin/cli_produce.exe
fdfs2qq_PID=/usr/local/webserver/fdfs2qq_mongo/etc/cli_produce_run.pid
...
case "$1" in
	...
	start)
		...
		nohup nice $fdfs2qq_BIN &> /dev/null 2>/tmp/cli_produce.error & echo $! > $fdfs2qq_PID
